...
// Fetch edit history
$hist_stmt = $conn->prepare("SELECT * FROM record_edit_history WHERE record_id=? ORDER BY edited_at DESC");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $upd = $conn->prepare("UPDATE donation_records SET
        ...
        if ($upd->execute()) {
            // Log to history
            $log = $conn->prepare("INSERT INTO record_edit_history
  ...
  <div class="bl-main">
    ...
    <div class="bl-page-header">
      <div>
        ...
        <p>Update information for record <strong style="color:#C0392B">#<?php echo htmlspecialchars($rec['record_id']); ?></strong></p>
      </div>
