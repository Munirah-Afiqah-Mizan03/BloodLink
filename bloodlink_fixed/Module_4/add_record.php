...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $recorded_by    = $current_user_display;
    ...
    if (empty($errors)) {
        ...
        // ── GET DONOR DB ID ──────────────────────────────────
        $d = $conn->prepare("SELECT id FROM bl_donors WHERE donor_id = ?");
        ...
        if (!$donor_row) {
            ...
            // ── GENERATE NEXT RECORD ID (FIXED) ─────────────
            // Uses MAX() on the numeric part so deletions or
            // out-of-order inserts never cause duplicates.
            $res      = $conn->query(
                "SELECT MAX(CAST(SUBSTRING(record_id, 4) AS UNSIGNED)) AS max_num
                 FROM bl_donation_records"
            );
            ...
            $ins = $conn->prepare(
                "INSERT INTO bl_donation_records
                 ...
                 // ON DUPLICATE KEY is a silent safety net for race conditions;
                 // the MAX() query above prevents duplicates in normal use.
            );
            $ins->bind_param(
                "siisississs",
                $record_id,
            ...
            if ($ins->execute()) {
                ...
                $_SESSION['flash'] = [
                    'type' => 'success',
                    'msg'  => "Record $record_id saved successfully."
                ];
          ...
          <div class="bl-field">
            ...
            <select name="blood_type" id="field-bloodtype">
              ...
              <option value="<?php echo $bt; ?>"
                <?php echo (($_POST['blood_type'] ?? '') === $bt) ? 'selected' : ''; ?>>
                <?php echo $bt; ?>
              </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="bl-field">
            ...
            <select name="status" id="field-status">
              ...
              <option value="<?php echo $s; ?>"
                <?php echo (($_POST['status'] ?? 'Pending') === $s) ? 'selected' : ''; ?>>
                <?php echo $s; ?>
              </option>
              <?php endforeach; ?>
            </select>
