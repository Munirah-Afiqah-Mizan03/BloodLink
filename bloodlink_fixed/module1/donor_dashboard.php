...
$pageTitle  = 'Dashboard';
$u = current_user();
$donor_db_id = $u['donor_id'];

// ── Fetch donor record (try by id first, then fallback by ic_number) ──
$donor = null;
if ($donor_db_id) {
    $stmt = $conn->prepare(
...
// If still no donor record, build a minimal one from session data
if (!$donor) {
    $donor = [
        'donor_id'      => '—',
...
if ($donor_db_id) {
    ...
    $last_don->bind_param("i", $donor_db_id);
    $last_don->execute();
...
if ($donor_db_id) {
    ...
    $cnt_stmt->bind_param("i", $donor_db_id);
    $cnt_stmt->execute();
    $total_donations = $cnt_stmt->get_result()->fetch_assoc()['c'];
}

// ── Upcoming bookings (pending or approved, future events) ────
$bk_stmt = $conn->prepare(
    "SELECT b.*, s.event_name, s.event_date, s.start_time, s.end_time, s.event_location
     ...
     WHERE b.donor_id = ? AND b.status IN ('pending','approved') AND s.event_date >= CURDATE()
     ORDER BY s.event_date ASC LIMIT 3"
);
$bk_stmt->bind_param("i", $u['id']);
$bk_stmt->execute();
...
<?php include 'sidebar.php'; ?>
  ...
  <div class="bl-main">
    ...
    <div class="bl-top-hero">
      <div>
        <h2><?php echo $greet; ?>, <?php echo htmlspecialchars($donor['first_name']); ?></h2>
          ...
          <div style="display:flex;flex-direction:column;gap:8px">
            ...
            <div style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius-sm);padding:10px 14px">
              ...
              <div style="margin-top:6px">
                <?php
                  $st = $bk['status'];
                  $st_cls = $st === 'approved' ? 'bl-badge-approved' : 'bl-badge-pending';
                ?>
                <span class="bl-badge <?php echo $st_cls; ?>"><?php echo ucfirst($st); ?></span>
              </div>
            </div>
            <?php endwhile; ?>
          </div>
